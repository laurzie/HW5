...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/laurzieHW5"
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        l = TodoItem.query.filter_by(description=item).first()
...
def delete(lst):
    selected_l = TodoList.query.filter_by(title=lst).first()
